...
public class HomeTownUI extends Activity implements OnWheelChangedListener {
	...
	private void updateAreas() {
		...
		if (mCitisDatasMap != null && mCurrentProviceName != null) {

			String[] tmpArray = mCitisDatasMap.get(mCurrentProviceName);
			if (tmpArray!=null&&tmpArray.length>0&&pCurrent < tmpArray.length) {
...
//				.show();
		...
		if("国外".equals(mCurrentProviceName)){
			Log.e("选择的地区", mCurrentProviceName);
			
			
		}else{
		Log.e("选择的地区", mCurrentProviceName + mCurrentCityName);
		}
		
		Intent intent = new Intent();
		intent.putExtra("mCurrentProviceName", mCurrentProviceName+"  "+mCurrentCityName);
		setResult(1025, intent);
		HomeTownUI.this.finish();

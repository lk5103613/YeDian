...
public class ImageAdapter extends BaseAdapter{  
	...
	public View getView(int position, View convertView, ViewGroup parent) {  
        ...
        if(convertView==null){  
           ...
           image = ((IndexUI)context).imagesCache.get(imageUrls.get(position % imageUrls.size())); //从缓存中读取图片  
           if(image==null){  
                //当缓存中没有要使用的图片时，先显示默认的图片  
                //image = ((IndexUI)context).imagesCache.get("background_non_load");  
                //异步加载图片  
...
//				mHandler.sendMessage(m);
            ...
            convertView.setTag(imageUrls.get(position % imageUrls.size()));  
       ...
       // textView.setBackgroundColor(Color.argb(200, 135, 135, 152));
         ...
         String img=imageUrls.get(position %len);
         //
        imageView.setImageBitmap(image);
        //设置缩放比例：保持原样  
        imageView.setScaleType(ImageView.ScaleType.FIT_XY); 
       // textView.setText(position % imgs.length+" sdfsdfdsfdf");
        ((IndexUI)context).changePointView(position % imageUrls.size());
...
//                       is.close();  
//                       ((IndexUI)context).imagesCache.put(params[0],image);   //把下载好的图片保存到缓存中
//                       	Message m = new Message();
//           				m.what = 0;
//           				mHandler.sendMessage(m);
//                   } catch (Exception e) {  
